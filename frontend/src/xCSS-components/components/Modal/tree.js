...
function mapDataToTreeData(data) {
    ...
    for (let key in data) 
    {
      ...
      if (key ==="attributes") {
        node.attributes = [];
        value.map(item=>{
          if((item.name == "class") || (item.name == "src") || (item.name == "alt")) {
...
function treeDataToJson(data) {
  ...
  function converter(dom) {
      const obj = {};
      if (dom.nodeType === Node.TEXT_NODE) {    // 3
          obj.nodeType = 3
          obj.tagName = "a"
          obj.textContent = dom.nodeValue;
          return obj;
      }
      if (dom.nodeType === Node.DOCUMENT_NODE) {    // 9
          //////console.log("document")
          dom = dom.documentElement;
      }

      obj.nodeType = dom.nodeType;
      if (dom.nodeType === Node.ELEMENT_NODE) {   // 1
...
function get_dev_content(nodeData)
{
  ...
  var treeData = [mapDataToTreeData(nodeData[0])];
  ...
  // console.log(json2html( JSON.stringify( converter_worked( rejson ), null, 4) ));
...
class Tree extends React.Component {
  ...
  constructor(props) {
    ...
    if(document.getElementsByClassName('canvas-panel')[0].children.length == 0) {
      var MyDiv1 = document.getElementsByClassName('canvas-panel')[0];
    } else {
      var MyDiv1 = document.getElementsByClassName('canvas-panel')[0].firstChild;
    }
    // var MyDiv1 = document.getElementById("tree-init");
    // MyDiv1.innerHTML = '';
    // console.log(MyDiv1.innerHTML)
    // console.log("===================DOM=====================");
    // console.log(MyDiv1)
    const json = JSON.stringify(converter(MyDiv1), null, 4);
    // console.log(converter(MyDiv1));
    // console.log(json);
    // console.log(json2html(json));
  ...
  handleTreeOnChange = (treeData) => {
    ...
    // useCoreDataStore.getState().addNewTreeElement(treeData);
    // console.log("===================change======================");
    // console.log(treeData);
    ...
    return (
      <div className="wrapper_tree">
        <div className="bar_wrapper_tree">
          ...
          {/* <button onClick={() => {get_dev_content(this.state.treeData);useCoreDataStore.getState().addNewTreeElement(this.state.treeData)}} >Add Tree</button> */}
          <button onClick={() => {useCoreDataStore.getState().addNewTreeElement(this.state.treeData);this.props.callback();}} >Update Tree</button>
          ...
          <button className="cancel-tree" onClick={this.props.callback} style={{ marginRight:'10px' }}>
            Cancel
          </button>
        </div>
                      ...
                      onClick={() =>
                        this.setState((state) => ({
                          treeData: addNodeUnderParent({
                            ...
                            newNode: {
                              nodeType: 1,
                              attributes: [{name:"class", value:"canvas-item"}] ,
                              title: `DIV`,
                              tagName: 'DIV',
                              children:[],
                            },
                    ...
                    onClick={() =>
                      this.setState((state) => ({
                        treeData: addNodeUnderParent({
                          ...
                          newNode: {
                            nodeType: 1,
                            attributes: [{name:"class", value:"canvas-item"}, {name:"alt", value:"upload the source file"}] ,
                            title: `IMG`,
                            tagName: 'IMG',
                            children:[],
                          },
