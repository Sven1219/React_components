...
export const useCoreDataStore = create(persist((set, get) => ({
  ...
  treeCreateAddElements: (data, treeIdInfo, id, size) =>{
    data.children.filter(item => item.attributes[0].value === 'canvas-item').map((child, index) => {
      const width = size/2;
      const height = size/2;
      ...
      if(child.children.filter(item => item.attributes[0].value === 'canvas-item').length != 0) {
